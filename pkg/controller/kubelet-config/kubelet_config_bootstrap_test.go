...
func TestRunKubeletBootstrap(t *testing.T) {
	for _, platform := range []configv1.PlatformType{configv1.AWSPlatformType, configv1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			mcs, err := RunKubeletBootstrap("../../../templates", &cfg, nil, cc, pools)
			require.NoError(t, err)
			require.Len(t, mcs, len(pools))

			for i := range pools {
				keyReg, _ := getManagedKeyKubelet(pools[i], nil)
				verifyKubeletConfigJSONContents(t, mcs[i], keyReg, cfg, cc.Spec.ReleaseImage)
			}
		})
	}
}

func verifyKubeletConfigJSONContents(t *testing.T, mc *mcfgv1.MachineConfig, mcName string, kubeCfg mcfgv1.KubeletConfig, releaseImageReg string) {
